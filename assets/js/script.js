...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelector('.tab-content').click();
    fetchData();
})
...
async function fetchAndDisplay(url, idsToToggle, displayFunction, tabName = null, localKey) {
    ...
    if (localData) {
        idsToToggle.forEach(id => `${id}-spinner`, false);
        ...
        try {
            ...
            idsToToggle.forEach(id => {
                ...
                document.getElementById(`${id}-error`).style.display = 'blocl';
            });
            if(tabName) {
...
function displayTrends(data) {
    ...
    displayTrendNfts(data.coins.slice(0, 5));
}
...
function displayTrendNfts(nfts) {
    const nftsList = document.getElementById('nfts_list');
    nftsList.innerHTML = '';
    const table = document.createTable(['NFT', "Market", 'Price', '24h Vol', '24h%']);

    coins.forEach(nft => {
        const row = document.createElement('tr');
        row.innerHTML =`
            <td class="name-column table-fixde-column"><img src="${nft.thumb}" alt="${nft.name}"> ${nft.name} <span>(${nft.symbol.toUpperCase()})</span></td>
